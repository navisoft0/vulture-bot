...
function playLine(p) {
  ...
  return `${arrow} ${esc(bits.join(" · "))}${p.rationale ? " — " + esc(p.rationale) : ""}`;
}
...
function candidateCard(c) {
  ...
  const flags = c.red_flags ? `<div class="flags">⚠️ ${esc(c.red_flags)}</div>` : "";
  return `<div class="card">
    ...
    <div class="subscores">
      <span>Thesis ${c.thesis}</span><span>Community ${c.community}</span>
      <span>News ${c.news}</span><span>Technicals ${c.technical}</span>
    </div>
...
const pages = {
  ...
  async tracker() {
    ...
    $("#tiles").innerHTML = `
      <div class="tile"><div class="n ok">${d.totals.HIT}</div><div class="l">hits</div></div>
      <div class="tile"><div class="n bad">${d.totals.MISS}</div><div class="l">misses</div></div>
      <div class="tile"><div class="n mid">${d.totals.WASH}</div><div class="l">washes</div></div>
      <div class="tile"><div class="n">${judged ? Math.round(d.totals.HIT / judged * 100) + "%" : "—"}</div><div class="l">hit rate</div></div>`;
    $("#open").innerHTML = d.open.length ? d.open.map(p => `<tr>
      <td><b>${esc(p.ticker)}</b></td><td>${playLine(p)}</td>
      <td>${esc(p.expiry || "open-ended")}</td><td>${when(p.promoted_at)}</td>
    ...
    $("#resolved").innerHTML = d.resolved.length ? d.resolved.map(r => `<tr>
      <td><b>${esc(r.ticker)}</b></td><td>${playLine(r)}</td>
      <td class="${r.verdict === "HIT" ? "ok" : r.verdict === "MISS" ? "bad" : "mid"}"><b>${esc(r.verdict)}</b></td>
      <td>${fmtPct(r.return_pct)}</td><td>${esc(r.method)}</td><td>${when(r.graded_at)}</td></tr>`).join("")
  ...
  async cramer() {
    ...
    $("#tiles").innerHTML = `
      <div class="tile"><div class="n ok">${d.totals.HIT}</div><div class="l">hits</div></div>
      <div class="tile"><div class="n bad">${d.totals.MISS}</div><div class="l">misses (inverse)</div></div>
      <div class="tile"><div class="n mid">${d.totals.WASH}</div><div class="l">washes</div></div>
      <div class="tile"><div class="n">${judged ? Math.round(d.totals.HIT / judged * 100) + "%" : "—"}</div><div class="l">hit rate</div></div>`;
    const stanceIcon = s => ({ buy: "🟢", sell: "🔴", avoid: "🔴", trim: "🟠", hold: "⚪" }[s] || "❔");
    $("#mentions").innerHTML = d.mentions.length ? d.mentions.map(m => `<tr>
      <td>${when(m.extracted_at)}</td><td><b>${esc(m.ticker)}</b></td>
      <td>${stanceIcon(m.stance)} ${esc(m.stance)}</td>
      <td>${m.verdict ? `<b class="${m.verdict === "HIT" ? "ok" : m.verdict === "MISS" ? "bad" : "mid"}">${m.verdict}</b> (α ${fmtPct(m.alpha_pct)})` : "⏳ open"}</td>
  ...
  async data() {
    const load = async () => {
      ...
      $("#rows").innerHTML = d.candidates.length ? d.candidates.map(c => `<tr>
        <td>${when(c.scored_at_utc)}</td><td><b>${esc(c.ticker)}</b></td>
        <td class="${scoreClass(c.composite)}"><b>${Number(c.composite).toFixed(2)}</b></td>
        <td>${c.thesis}/${c.community}/${c.news}/${c.technical}</td>
        <td>${c.posted ? "📣" : c.radar ? "👀" : ""}</td>
